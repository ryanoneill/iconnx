...
8. **CPU/GPU round-half-to-even parity** — DynamicQuantizeLinear's ORT-conformance correctness hinges on bit-identical rounding between CPU and GPU paths. CPU uses Rust's `f32::round_ties_even()` (Rust 1.77+); CUDA's `__float2int_rn()` intrinsic is round-to-nearest-even by IEEE-754 default and matches. **M4.5 implementation must verify** that no intermediate path uses `__float2int_rd` / `__float2int_ru` / `__float2int_rz` (round-down/up/zero) — only `__float2int_rn` or equivalent half-to-even rounding. Per-input ORT-conformance tests will catch any drift, but the call-site-level audit is cheap and worth doing once during the kernel write.
