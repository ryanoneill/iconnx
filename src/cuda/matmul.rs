...
use garboard::{GemmConfig, GemmStridedBatchedConfig, Transpose};
...
pub fn gpu_matmul(
    ...
    let config = GemmConfig::<f32> {
        ...
        k: k as i32,
        alpha: 1.0,
        lda: n as i32, // leading dim of first operand (B in our convention)
        ldb: k as i32, // leading dim of second operand (A)
        beta: 0.0,
        ldc: n as i32,
    };
...
///
/// Broadcasts the 2D matrix A across all batches of B via strided batched
/// GEMM with `stride_a = 0`.
pub fn gpu_matmul_2d_3d(
    ...
    let config = GemmStridedBatchedConfig::<f32> {
        ...
        alpha: 1.0,
        lda: n as i32,
        stride_a: stride_b_ours, // swapped — cuBLAS's "A" is our B
        ldb: k as i32,
        stride_b: stride_a_ours, // swapped — cuBLAS's "B" is our A (broadcast)
        beta: 0.0,
        ldc: n as i32,
        stride_c,
...
///
/// For A[batch, M, K] @ B[batch, K, N] = C[batch, M, N]. Uses strided
/// batched GEMM.
pub fn gpu_batched_matmul(
    ...
    let config = GemmStridedBatchedConfig::<f32> {
        ...
        alpha: 1.0,
        lda: n as i32,
        stride_a: stride_b_ours, // cuBLAS's "A" is our B
        ldb: k as i32,
        stride_b: stride_a_ours, // cuBLAS's "B" is our A
        beta: 0.0,
        ldc: n as i32,
        stride_c,
        batch_count: batch as i32,
