...
//!
//! Reduces allocation overhead by reusing GPU memory blocks.
//! Tracks freed memory by (dtype, size) and returns pooled memory
//! when available, falling back to fresh allocation when not.
...
///
/// Rounds up to power-of-2 boundaries to improve hit rates.
/// Similar-sized allocations share the same bucket, reducing fragmentation.
///
...
/// exact sizes. For large allocations, the overhead percentage is small anyway.
#[inline]
fn size_class(len: usize) -> usize {
    // Use power-of-2 bucketing for allocations >= 4096 elements.
    // This improves pool hit rates by grouping similar-sized allocations.
    // Small allocations use exact matching to avoid excessive memory overhead.
    if len < 4096 {
        len
    } else {
...
///
/// Maintains separate pools for each data type (f32, i64, i32).
/// Memory is keyed by size-class bucket (power-of-2 rounding) for better reuse.
pub struct GpuMemoryPool {
    /// Pool of f32 slices, keyed by size-class bucket
    f32_pool: HashMap<usize, Vec<CudaSlice<f32>>>,
    /// Pool of i64 slices, keyed by size-class bucket
    i64_pool: HashMap<usize, Vec<CudaSlice<i64>>>,
    /// Pool of i32 slices, keyed by size-class bucket
    i32_pool: HashMap<usize, Vec<CudaSlice<i32>>>,
...
impl GpuMemoryPool {
    ...
    /// Get or allocate a Float32 CudaSlice
    ///
    /// Returns a pooled slice if available, otherwise allocates fresh.
    /// Note: The returned slice may be larger than `len` due to size-class bucketing.
    /// Pooled slices are zeroed before being returned to ensure consistent behavior.
    pub fn get_f32(
        &mut self,
        ...
        *self.alloc_counts.entry(len).or_insert(0) += 1;
        if let Some(slices) = self.f32_pool.get_mut(&bucket) {
            ...
            // This handles the case where different sizes hash to the same bucket
            if let Some(idx) = slices.iter().position(|s| s.len() >= len) {
                let mut slice = slices.swap_remove(idx);
                self.hits += 1;
                // Zero the slice to ensure consistent behavior with fresh allocations
    ...
    /// Get or allocate an Int64 CudaSlice
    ///
    /// Note: The returned slice may be larger than `len` due to size-class bucketing.
    /// Pooled slices are zeroed before being returned to ensure consistent behavior.
    pub fn get_i64(
        ...
        ctx: &IconnxCudaContext,
        len: usize,
    ) -> Result<CudaSlice<i64>, CudaError> {
        let bucket = size_class(len);
    ...
    /// Get or allocate an Int32 CudaSlice
    ///
    /// Note: The returned slice may be larger than `len` due to size-class bucketing.
    /// Pooled slices are zeroed before being returned to ensure consistent behavior.
    pub fn get_i32(
    ...
    pub fn return_f32(&mut self, slice: CudaSlice<f32>) {
        let len = slice.len();
    ...
    pub fn return_i64(&mut self, slice: CudaSlice<i64>) {
        let len = slice.len();
    ...
    pub fn return_i32(&mut self, slice: CudaSlice<i32>) {
        let len = slice.len();
    ...
    /// Get sizes that are allocated but never returned (optimization opportunities)
    ...
    /// before it's needed. Call this after creating the executor but before
    /// running inference.
    ///
    /// The sizes are based on empirical analysis of Kokoro TTS model allocations.
    /// Note: With size-class pooling, these sizes are rounded up to power-of-2 buckets.
    /// `count` specifies how many slices to pre-allocate per size (default 2).
    pub fn warm(&mut self, ctx: &IconnxCudaContext, count: usize) -> Result<(), CudaError> {
        // Common sizes observed in Kokoro TTS model (high allocation frequency)
        // These will be rounded up to size-class buckets (128, 256, 512, 1024, 4096, 8192, 16384, 32768, 65536)
    ...
    ///
    /// Use this when you know the specific allocation pattern for your model.
    /// `sizes` is a list of (element_count, count) pairs.
    /// Note: Sizes are rounded up to size-class buckets.
    pub fn warm_sizes(
        &mut self,
        ctx: &IconnxCudaContext,
        sizes: &[(usize, usize)],
