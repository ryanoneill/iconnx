    ...
    if num_shape == var_shape {
        ...
        unsafe {
            ctx.stream()
                ...
                .arg(&GbKernelArg::new(numerator.data_f32()?))
                .arg(&GbKernelArg::new(variance.data_f32()?))
                .arg(&eps)
                .arg(&n)
        ...
        if t_dim > 1 && is_per_channel_x && is_per_channel_c && is_a_trivial {
            ...
            unsafe {
                ctx.stream()
                    .launch_builder(func)
                    .arg(&GbKernelArg::new_mut(out.data_f32_mut()?))
                    .arg(&GbKernelArg::new(b.data_f32()?))    // input (full shape)
                    .arg(&GbKernelArg::new(x.data_f32()?))    // scale (per-channel)
                    .arg(&GbKernelArg::new(c.data_f32()?))    // bias (per-channel)
                    .arg(&n)
                    .arg(&t_dim)
                    .launch(elementwise_config(n))
                    .map_err(|e| {
    ...
    if w0.shape() == shape && w1.shape() == shape && b.shape() == shape {
        ...
        unsafe {
            ctx.stream()
                ...
                .arg(&GbKernelArg::new_mut(out.data_f32_mut()?))
                .arg(&GbKernelArg::new(x.data_f32()?))
                .arg(&GbKernelArg::new(w0.data_f32()?))
                .arg(&GbKernelArg::new(w1.data_f32()?))
                .arg(&GbKernelArg::new(b.data_f32()?))
                .arg(&p)
                .arg(&n)
                .launch(elementwise_config(n))
